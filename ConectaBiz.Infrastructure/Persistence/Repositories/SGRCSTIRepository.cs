               ...
               await context.OpenAsync();
                using (var command = new NpgsqlCommand(query, context))
                {
                    using (var reader = command.ExecuteReader())
                    {
                        ...
                        {
                            var empresa = Empresa.Materializar(
                                reader.IsDBNull(reader.GetOrdinal("CodSgrCsti")) ? null : reader.GetInt32(reader.GetOrdinal("CodSgrCsti")),
                                reader.GetString(reader.GetOrdinal("RazonSocial")),
                                reader.IsDBNull(reader.GetOrdinal("NombreComercial")) ? null : reader.GetString(reader.GetOrdinal("NombreComercial")),
                                reader.IsDBNull(reader.GetOrdinal("NumDocContribuyente")) ? null : reader.GetString(reader.GetOrdinal("NumDocContribuyente")),
